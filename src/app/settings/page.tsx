...
import { useState, useEffect } from "react"; // 1. Added these
...
export default function SettingsPage() {
  ...
  const handleExport = () => {
    ...
    const csvContent = "data:text/csv;charset=utf-8,"
      + "CreatedAT,Description,Category,UpdatedAt,Type,Amount\n"
      + transactions.map(t => `${t.date},"${t.description.replace(/"/g, '""')}",${t.category},${t.updated_at},${t.type},${t.amount}`).join("\n");
        ...
        <CardContent className="flex gap-5 flex-col">
          ...
          <div className="flex items-center justify-between p-4 border rounded-lg bg-secondary/50">
            ...
            <Button variant="outline" size="sm" asChild>
              {/* 3. Wrap in isMounted check */}
              {!isMounted ? (
                <span className="flex items-center">
                  <Download className="mr-2 h-4 w-4" /> Preparing...
                </span>
              ) : (
                <PDFDownloadLink
                  document={
