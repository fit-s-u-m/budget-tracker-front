...
import { CategoryName, Transaction } from "@/lib/types";
...
import { useAddTransaction, useUpdateTransaction, useCategoies } from "@/hook/useBudget";
...
export function TransactionForm({ isOpen, onClose, initialData }: TransactionFormProps) {
  ...
  const addTransaction = useAddTransaction(user_id || "");
  const getCategories = useCategoies()
  ...
  const onSubmit = async (data: TransactionFormData) => {
    if (initialData) {
      ...
      const updatedCategoryId = await getCategoryId(updated.category)
